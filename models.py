...
def build_model(name, load_checkpoint=None, **kwargs) -> nn.Module:
    """
    Instantiate a backbone class.

    Args:
        name: model name
        load_checkpoint: path of checkpoint

    Returns:
        model (nn.Module): the model for extracting features.
    """
    if name not in BACKBONES.keys():
        raise KeyError("Invalid model, got '{}', but expected to be one of {}".format(name, BACKBONES.keys()))

    model = BACKBONES[name](**kwargs)

...
def build_reranker(name, **kwargs):
    if name not in RERANKERS.keys():
        raise KeyError("Invalid reranker, got '{}', but expected to be one of {}".format(name, RERANKERS.keys()))
    reranker = RERANKERS[name](**kwargs)
    return reranker